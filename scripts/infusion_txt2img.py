...
from helpers import chunk, load_model_from_config
from helpers import sample as advanced_sample

def find_word_position(sentence, replace_word, target_word):
    sentence = sentence.replace('{}',replace_word)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="cat",
        help="the concepts for custom generation"
    )
    parser.add_argument(
        "--replace_word",
        type=str,
        default="cat",
        help="the concepts to replace {}"
    )
    parser.add_argument(
        "--outdir",
        type=str,
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/infusion_inference.yaml",
    ...
    parser.add_argument(
        "--ckpt",
        type=str,
        default="/home/zwl/codes/Key-Locked-Rank-One-Editing-for-Text-to-Image-Personalization-main/ckpt/sd-v1-5.ckpt",
    ...
    if n_concepts > 1:
        config.model.target = 'infusion.infusion.MultiConceptsInfusion'
    ...
    if not opt.from_file:
        prompt = opt.prompt
        prompt = prompt.replace('{}',opt.replace_word)
    ...
    concept_token_idx = find_word_position(prompt, opt.replace_word, opt.target_word)-1
                ...
                for n in trange(opt.n_iter, desc="Sampling"):
                    for data_i in tqdm(range(len(data_concept)), desc="data"):
                        ...
                        if opt.scale != 1.0:
                            ...
                            uc = dict(c_crossattn=encoding_uc,concept_token_idx=(-1)*torch.ones(batch_size).cuda(),
                        ...
                        c = dict(c_crossattn=encoding,c_super=encoding_superclass,concept_token_idx=concept_token_idx*torch.ones(batch_size).cuda())

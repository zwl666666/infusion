...
from infusion.roe import ROELinear, roe_to_mc_roe
from infusion.dataset import PROMPT_TEMPLATE
...
class Infusion(LatentDiffusion):
    def __init__(
            self,
    ...
    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        ...
        self.target_input.data = self.cond_stage_model.encode('A * wearing headphones', embedding_manager=self.embedding_manager)
        eps = diffusion_model(
            ...
            concept_token_idx = cond['concept_token_idx'],
        )
        return eps
    ...
    def configure_optimizers(self):
        ...
        opt = torch.optim.AdamW(
            [{"params": target_output_params}], #{"params": embedding_params, "lr": self.embedding_lr}
...
# inference only
class MultiConceptsInfusion(LatentDiffusion):
    def __init__(
            self,
            personalization_config,
            C_inv_path='./ckpt/C_inv.npy',
        ...
        self.register_buffer('target_inputs',
                             torch.empty(self.n_concepts,77, self.model.diffusion_model.context_dim, dtype=torch.float32))
    ...
    def init_from_personalized_ckpt(self, path_list):
        ...
        for i, path in enumerate(path_list):
            ...
            self.target_inputs= sd_target_input
